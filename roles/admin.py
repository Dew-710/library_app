...
from database.models import get_books, get_book_by_id, add_book, update_book, delete_book, get_users, get_borrow_records, get_borrowed_books_today, get_categories, get_db_connection
...
class AdminWindow:
    def __init__(self, root, user):
        self.root = root
        self.user = user
        self.root.title("Quản lý Admin - Thư viện THM")

        try:
            self.root.state('zoomed')  # Sẽ thành công trên Windows, một số Linux
        except Exception:
            try:
                self.root.attributes('-zoomed', True)  # Một số Linux hỗ trợ
            except Exception:
                # Cách cuối cùng: phóng to cửa sổ theo kích thước màn hình
                self.root.geometry(f"{self.root.winfo_screenwidth()}x{self.root.winfo_screenheight()}+0+0")
        self.root.resizable(False, False)

        sidebar_frame = tk.Frame(self.root, width=300, bg="#6B7280")
    ...
    def init_pages(self):
        ...
        tk.Button(search_frame, text="Xóa sách", command=self.delete_book_gui, bg="green", fg="white").grid(row=2,column=4,padx=5,pady=5)
        tk.Button(search_frame, text="Mượn sách", command=self.borrow_book, bg="green", fg="white").grid(row=2, column=5, padx=5, pady=5)
    ...
    def on_book_select(self, event):
        ...
        if selected_item:
            ...
            book = get_book_by_id(book_id)#Lấy sách từ id đó
            if book:
    ...
    def confirm_borrow(self, window, book_id, user_name, users):
        ...
        cursor = conn.cursor()
        try:
            borrow_date = datetime.now().date()  # Ngày mượn dựa trên thời gian hiện tại
            due_date = borrow_date + timedelta(days=14)  # Hạn trả là 2 tuần (14 ngày)
            cursor.execute(
